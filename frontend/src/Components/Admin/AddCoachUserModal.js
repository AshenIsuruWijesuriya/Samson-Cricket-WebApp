...
const AddCoachUserModal = ({ onClose, onUserAdded }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            console.log('Coach user registered:', response.data);
            Swal.fire({ title: 'Success!', text: 'Coach user successfully added!', icon: 'success', confirmButtonText: 'OK' });
            onUserAdded();
            onClose();
        } catch (error) {
            console.error('Error registering coach user:', error.response?.data || error.message);
    ...
    return (
        <div className="admin-user-modal-overlay">
            <div className="admin-user-modal-content">
                <h2 className="admin-user-modal-title">Add New Coach User</h2>
...
export default AddCoachUserModal;