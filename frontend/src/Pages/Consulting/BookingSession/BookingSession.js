...
const BookingSession = ({ selectedType, consultants, filteredConsultants, loading, error }) => {
    ...
    }, [convertTo24Hour]); // Included convertTo24Hour in dependency array

    // Validate date and time against consultant availability
    const validateDateTime = useCallback((date, time, availabilityData = selectedConsultantAvailability) => {
        const errors = {};
        ...
        return errors;
    }, [selectedConsultantAvailability]);
    ...
    }, [formData.consultantId, consultants, parseAvailabilityString, validateDateTime, formData.date, formData.time]);
