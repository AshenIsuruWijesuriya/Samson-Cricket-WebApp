...
const BookingSession = ({ selectedType, consultants, filteredConsultants, loading, error }) => {
    ...
    const parseAvailabilityString = useCallback((availabilityStr) => {
    ...
    const validateDateTime = useCallback((date, time, availabilityData) => {
        const errors = {};
        ...
        if (selectedDate < today) {
            ...
            const availableSlots = availabilityData ? availabilityData.filter(slot => slot.dayOfWeek === dayOfWeek) : [];
    ...
    }, [formData.consultantId, consultants, formData.date, formData.time, parseAvailabilityString, validateDateTime]);
    ...
    const handleChange = (e) => {
        ...
        if (name === 'date' || name === 'time') {
            ...
            validateDateTime(dateValue, timeValue, selectedConsultantAvailability);
        }
    };
    ...
    const handleSubmit = async (e) => {
        ...
        const errors = validateDateTime(formData.date, formData.time, selectedConsultantAvailability);
